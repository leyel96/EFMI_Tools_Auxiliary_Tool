# Step 3 재시도 - EUC-KR 인코딩으로 수정

with open('mesh_extractor_gui.py', 'r', encoding='euc-kr') as f:
    content = f.read()
...
if old_start in content:
    ...
    print("1. _start_extracted_import 패턴을 찾을 수 없음")
    # 디버깅: 실제 내용 출력
    idx = content.find('def _start_extracted_import')
    if idx >= 0:
        print("실제 내용:")
...
if old_run in content:
    ...
    print("2. _run_extracted_import_thread 패턴을 찾을 수 없음")
    idx = content.find('def _run_extracted_import_thread')
    if idx >= 0:
        print("실제 내용:")
        print(content[idx:idx+500])

with open('mesh_extractor_gui.py', 'w', encoding='euc-kr') as f:
    f.write(content)
